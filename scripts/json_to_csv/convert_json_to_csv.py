...
def process_ticket(ticket_data):
    ...
    try:
        ...
        return {
            ...
            'ticket_type': clean_text(safe_get(ticket_data, 'type')),
            'subject': clean_text(safe_get(ticket_data, 'subject')),
            'description': clean_text(safe_get(ticket_data, 'description')),
            'details': format_comments(safe_get(ticket_data, 'comments', default=[])),
            ...
            # Handle the 'collaborator' field which may be a list
            'collaborator_name': safe_get(ticket_data, 'collaborator', 0, 'name') if isinstance(safe_get(ticket_data, 'collaborator'), list) else safe_get(ticket_data, 'collaborator', 'name'),
            'collaborator_email': safe_get(ticket_data, 'collaborator', 0, 'email') if isinstance(safe_get(ticket_data, 'collaborator'), list) else safe_get(ticket_data, 'collaborator', 'email'),
            'categories': tags_str,
            ...
            'agent_wait_time_in_minutes': safe_get(ticket_data, 'metric_set', 'agent_wait_time_in_minutes', 'business'),
            'dates_assigned_at': safe_get(ticket_data, 'dates', 'assigned_at'),
            'dates_solved_at': safe_get(ticket_data, 'dates', 'solved_at'),
            'dates_latest_comment_added_at': safe_get(ticket_data, 'dates', 'latest_comment_added_at'),
            'received_via_channel': safe_get(ticket_data, 'via', 'channel'),
        }
